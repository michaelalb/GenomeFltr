...
class EMAIL_CONSTS:
    def create_title(state, job_name):
        if state == State.Finished:
            if job_name != "":
                return f'Genomefltr {job_name} - Job Finished'
            return f'Genomefltr - Job Finished'
        elif state == State.Crashed:
            if job_name != "":
                return f'Genomefltr {job_name} - Job Crashed'
            return f'Genomefltr - Job Crashed'
        elif state == State.Init:
            if job_name != "":
                return f'Genomefltr {job_name} - Job Initialized'
            return f'Genomefltr - Job Initialized'
        elif state == State.Running:
            if job_name != "":
                return f'Genomefltr {job_name} - Job Running'
            return f'Genomefltr - Job Running'
        else:
            return f'unknown state in create_title at EMAIL_CONSTS'
    
    
    CONTENT_CRASHED_DOWNLOAD_PROCESS =  '''Thank you, for using GenomeFLTR.\nWe are sorry for the inconvenience, but the genome donwload process crashed, pleasy verify your input (verify the taxa ids are correct and their genomes are available online) and let us know if this happens again at: edodotan@mail.tau.ac.il'''
    CONTENT_CRASHED_POST_PROCESS =  '''Thank you, for using GenomeFLTR.\nWe are sorry for the inconvenience, but the post process crashed, pleasy verify your input and let us know if this happens again at: edodotan@mail.tau.ac.il'''
    CONTENT_CRASHED_KRAKEN_SEARCH =  '''Thank you, for using GenomeFLTR.\nWe are sorry for the inconvenience, but the kraken process crashed, pleasy verify your input and let us know if this happens again at: edodotan@mail.tau.ac.il'''

    CONTENT_KRAKEN_SEARCH =  '''Thank you, for using GenomeFLTR.\nYour results are at:\nhttps://genomefltr.tau.ac.il/process_state/{process_id}\nPlease remember to cite us'''
    CONTENT_POST_PROCESS =  '''Thank you, for using GenomeFLTR.\nThe filtered reads can be downloaded from:\nhttps://genomefltr.tau.ac.il/download_file/{process_id}\nPlease remember to cite us'''
    SUBMITTED_TITLE = '''Genomefltr {job_name} - Job Submitted'''
    SUBMITTED_CONTENT = '''Thank you, for using GenomeFLTR.\nYour job has been submitted, you can check its status at:\nhttps://genomefltr.tau.ac.il/process_state/{process_id}\nAn update will be sent upon completion.'''


class UI_CONSTS:
    SPECIES_FORM_PREFIX = 'species' # prefix for the form of the custom DB input (passed from home to __init__)
    ACCESSION_FORM_PREFIX = 'accession' # prefix for the form of the custom DB input (passed from home to __init__)
    ...
    class UI_Errors(Enum):
        ...
        KRAKEN_JOB_CRASHED = 'Your process crashed... Make sure your input is valid'
        DOWNLOAD_JOB_CRASHED = 'Your download process crashed... Make sure your input download parameters are valid. Including the availability of the required genomes'

    ERROR_CONTACT_INFO = 'For more information, or any other inquiries, please contact edodotan@mail.tau.ac.il'
