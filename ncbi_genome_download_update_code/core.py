...
# acession_numbers parameter was added by Edo - 20.07.22
def download(acession_numbers=[], **kwargs):
    ...
    return config_download(acession_numbers, config)
...
# acession_numbers parameter was added by Edo - 20.07.22
def config_download(acession_numbers: list, config):
    ...
    try:
        download_candidates = select_candidates(acession_numbers, config)
...
# acession_numbers parameter was added by Edo - 20.07.22
def select_candidates(acession_numbers, config):
    ...
    for group in config.groups:
        ...
        for entry in filter_entries(acession_numbers, entries, config):
...
# acession_numbers parameter was added by Edo - 20.07.22
def filter_entries(acession_numbers, entries, config):
    ...
    new_entries = []
    #added by Edo 17.05
    # updated by Edo 20.07.22
    #########
    NUM_GENOMES2DOWNLOAD_PER_SPECIE = 3
    random_entries = []
    specific_entries2download = []
    taxaid_dict = {}
    print(f"taxaid = {config.taxids}, acession_numbers = {acession_numbers}")
    #########
    for entry in entries:
        ...
        #########
        # added by Edo 17.05
        # updated by Edo 20.07
        # download only 3 genomes
        taxa = entry.get('taxid', '0')
        taxaid_dict[taxa] = taxaid_dict.get(taxa, 0) + 1
        # donwload only entries with less than 3 entries
        entry_acession_number = entry.get('assembly_accession')
        if entry_acession_number in acession_numbers:
            specific_entries2download.append(entry)
        if taxaid_dict[taxa] > NUM_GENOMES2DOWNLOAD_PER_SPECIE:
            continue
        
        random_entries.append(entry)
    
    specific_entries2download.extend(random_entries)
    new_entries = specific_entries2download[:NUM_GENOMES2DOWNLOAD_PER_SPECIE]
    print(f'config.output = {config.output}')
    #for summary write the speices accession_number to file
    if len(new_entries) > 0:
        with open(os.path.join(config.output, 'genome_download_summary.txt'), 'a+') as f:
            acession_numbers_str = ','.join([x.get('assembly_accession') for x in new_entries])
            f.write(f"{new_entries[0].get('taxid')}: {acession_numbers_str}\n")
    for entry in new_entries:
        print(f"donwloading taxid = {entry.get('taxid')}, accession_number = {entry.get('assembly_accession')}")
    ########
    return new_entries
